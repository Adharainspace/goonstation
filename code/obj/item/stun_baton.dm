...
/obj/item/baton
	...
	var/flipped = false //is it currently rotated so youre grabbing it by the head?
...
/*/obj/item/baton/examine()
	...
	if (src.uses_charges != 0 && src.uses_electricity != 0)
		if (!src.cell || !istype(src.cell))
			...
			. += "The baton is turned [src.status ? "on" : "off"]. There are [src.cell.charge]/[src.cell.max_charge] PUs left! Each stun will use [src.cost_normal] PUs."
*/
/obj/item/baton/emp_act()
	if (src.uses_charges != 0 && src.uses_electricity != 0)
		src.status = 0
		src.process_charges(-INFINITY)
	return

/obj/item/baton/process()
...
/obj/item/baton/proc/update_icon()
	if (!src || !istype(src))
		return
...
/obj/item/baton/proc/can_stun(var/requires_electricity = 0, var/amount = 1, var/mob/user)
	if (!src || !istype(src))
		return 0
...
/obj/item/baton/proc/regulate_charge()
	if (!src || !istype(src))
		return
...
/obj/item/baton/proc/process_charges(var/amount = -1, var/mob/user)
	...
	src.update_icon()
	if(istype(user)) // user can be a Securitron sometimes, scream
		user.update_inhands()
	return

/obj/item/baton/proc/charge(var/amt)
	if(src.cell)
...
/obj/item/baton/proc/use_stamina_stun()
	if (!src || !istype(src))
		return 0
...
/obj/item/baton/proc/do_stun(var/mob/user, var/mob/victim, var/type = "", var/stun_who = 2)
...
/obj/item/baton/attack_self(mob/user as mob)
...
/obj/item/baton/attackby(obj/item/b as obj, mob/user as mob)
...
/obj/item/baton/proc/log_cellswap(var/mob/user as mob, var/obj/item/ammo/power_cell/C)
...
/obj/item/baton/intent_switch_trigger(var/mob/user)
	src.do_flip_stuff(user, user.a_intent)

/obj/item/baton/attack_hand(var/mob/user)
	if (src.flipped && user.a_intent != "harm")
		...
		src.do_flip_stuff(user, "harm", true)
	..()

/obj/item/baton/proc/do_flip_stuff(var/mob/user, var/intent, var/no_message = false)
	if (intent == "harm")
		if (src.flipped)
			return
		src.flipped = true
...
/obj/item/baton/ntso
	...
	can_swap_cell = 0
	status = 0
	item_function_flags = null //dont inherit intent flipping
...
/obj/item/baton/ntso/attack_self(mob/user as mob)
	src.add_fingerprint(user)
...
/obj/item/baton/ntso/update_icon()
	if (!src || !istype(src))
		return
	switch (src.state)
		if (CLOSED_AND_OFF)
...
/obj/item/baton/ntso/throw_impact(atom/A)
	if(isliving(A))
...
/obj/item/baton/ntso/intent_switch_trigger(var/mob/user)
	return
